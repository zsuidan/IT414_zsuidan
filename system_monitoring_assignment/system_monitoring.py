...
#Obtains the current hour and minutes
hour = raw_date.strftime('%H:%M')

#Attempts to open the script_config.json file
try:
    with open("text_files/script_config.json") as config_json:
        ...
        #Obtains the seconds since the last text alert sent
        current_time = time.time()
        last_text_time = config_data['last_text']
        seconds_since_last_text = current_time - float(last_text_time)

        #If over an hour (3600 seconds) has passed since the last text alert, checks system again
        if seconds_since_last_text >= 3600:
            ...
            #If resource usage exceeds a certain threshold, adds alert information to alert_message and sets send_alert to True
            send_alert = False
            alert_message = date + "\n"
            ...
            #If an issue is detected, sends the alert message as a text
            if send_alert:
...
#If the script_config.json file is not found, prints an error and exits
except:
